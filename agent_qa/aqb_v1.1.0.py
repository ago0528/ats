...
실행:
streamlit run aqb_v1.1.0.py --server.address 127.0.0.1 --server.port 8502
...
def _truncate(v: str, limit: int = 200) -> str:
    if len(v) <= limit:
        return v
    return v[: limit - 1] + "…"


def _build_worker_insights(sse: Dict[str, Any]) -> Dict[str, str]:
    workers = sse.get("workers") or []
    worker_ms_map = sse.get("worker_ms_map") or {}

    if not isinstance(workers, list):
        workers = []
    if not isinstance(worker_ms_map, dict):
        worker_ms_map = {}

    by_type_count: Dict[str, int] = {}
    typed_rows: List[Tuple[str, str, Optional[float], str]] = []
    # tuple: (type, label, ms, assistant_message)
    for w in workers:
        if not isinstance(w, dict):
            continue
        w_type = _clean_text(w.get("type"))
        if not w_type:
            continue

        seq = by_type_count.get(w_type, 0)
        by_type_count[w_type] = seq + 1
        label = f"{w_type}#{seq + 1}" if seq > 0 else w_type

        ms = _to_float_or_none(w.get("ms"))
        if ms is None:
            ms = _to_float_or_none(worker_ms_map.get(f"{w_type}#{seq}"))
        if ms is None:
            ms = _to_float_or_none(worker_ms_map.get(w_type))

        msg = ""
        output = w.get("output")
        if isinstance(output, dict):
            msg = _clean_text(output.get("assistantMessage") or output.get("assistant_message"))

        typed_rows.append((w_type, label, ms, msg))

    worker_chain = " -> ".join([row[1] for row in typed_rows])
    worker_ms_text = " | ".join(
        [f"{label}:{ms:.1f}" for _, label, ms, _ in typed_rows if ms is not None]
    )
    worker_answer_text = " | ".join(
        [f"{label}: {_truncate(msg)}" for _, label, _, msg in typed_rows if msg]
    )

    final_worker = ""
    final_worker_ms = ""
    for _w_type, label, ms, msg in reversed(typed_rows):
        if msg:
            final_worker = label
            final_worker_ms = f"{ms:.1f}" if ms is not None else ""
            break

    return {
        "호출 워커 수": str(len(typed_rows)),
        "호출 워커 체인": worker_chain,
        "워커별 소요시간(ms)": worker_ms_text,
        "워커별 답변 요약": worker_answer_text,
        "최종 응답 워커": final_worker,
        "최종 응답 워커 시간(ms)": final_worker_ms,
    }
                        ...
                        async with aiohttp.ClientSession(connector=connector, timeout=_timeout) as session:
                            ...
                            for fut in asyncio.as_completed(tasks):
                                ...
                                if sse is None:
                                    ...
                                    worker_insights = _build_worker_insights(sse)
                                    for k, v in worker_insights.items():
                                        res_df.at[idx, k] = v
