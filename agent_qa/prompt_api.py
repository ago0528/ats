...
class AxPromptApiClient:
    ...
    def __init__(
        ...
        Args:
            ...
            environment: 환경 이름 (DV, QA, ST, PR) - ST/PR 환경에서 인증 헤더 사용 여부 판단용
    ...
    def _get_auth_headers_if_needed(self) -> Optional[dict]:
        """
        환경에 따라 인증 헤더 반환 (ST, PR 환경에서만 필요)

        Returns:
            인증 헤더 딕셔너리 (ST/PR 환경일 때) 또는 None
        """
        # ST, PR 환경에서만 인증 헤더 필요
        if self.environment in ["ST", "PR"]:
            # 토큰이 있으면 값 포함, 없으면 빈 값으로 헤더 전송
            auth_token = self.retention_token if self.retention_token else ""
            mrs_session = self.mrs_session if self.mrs_session else ""
            cms_token = self.cms_access_token if self.cms_access_token else ""
