...
const spinStyle = {
    animation: 'aqb-icon-spin 1s linear infinite',
};
const KNOWN_ICONS = {
    FormOutlined: {
        name: 'form',
...
function normalizeAttributes(attrs) {
    ...
    Object.entries(attrs).forEach(([name, value]) => {
        if (typeof value === 'string') {
            result[toReactAttrName(name)] = value;
        }
    });
    return result;
}
function renderNode(node) {
    if (!node.children || node.children.length === 0) {
        return createElement(node.tag, { ...normalizeAttributes(node.attrs) });
    }
    return createElement(node.tag, {
        ...normalizeAttributes(node.attrs),
        fill: 'currentColor',
    }, ...(node.children || []).map((child) => renderNode(child)));
}
function makeIcon(def) {
    return ({ className, style, title, spin }) => {
        const mergedStyle = {
            ...iconContainer,
            ...(spin ? spinStyle : {}),
            ...(style ?? {}),
        ...
        const svgProps = {
            ...normalizeAttributes(def.icon.attrs),
            width: '1em',
            height: '1em',
            fill: 'currentColor',
        };
        return (_jsx("span", { className: className, style: mergedStyle, title: title, "aria-hidden": "true", children: createElement('svg', {
                ...svgProps,
                style: {
                    width: '100%',
                    height: '100%',
                },
            }, ...def.icon.children.map((node) => renderNode(node))) }));
    };
}
export const FormOutlined = makeIcon(KNOWN_ICONS.FormOutlined);
export const QuestionCircleOutlined = makeIcon(KNOWN_ICONS.QuestionCircleOutlined);
export const RobotOutlined = makeIcon(KNOWN_ICONS.RobotOutlined);
export const CheckCircleOutlined = makeIcon(KNOWN_ICONS.CheckCircleOutlined);
export const FileExcelOutlined = makeIcon(KNOWN_ICONS.FileExcelOutlined);
export const InfoCircleOutlined = makeIcon(KNOWN_ICONS.InfoCircleOutlined);
export const LoadingOutlined = makeIcon(KNOWN_ICONS.LoadingOutlined);
export const RocketOutlined = makeIcon(KNOWN_ICONS.RocketOutlined);
export const CloseCircleOutlined = makeIcon(KNOWN_ICONS.CloseCircleOutlined);
