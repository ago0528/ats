...
### 2.4 구현 구조 메모 (리팩토링 기준)

화면 IA/동작은 유지한 채 구현 구조는 아래 원칙으로 분리한다.

1. `*Page.tsx`는 라우팅 파라미터 해석과 섹션 조합 오케스트레이션만 담당한다.
2. 서버 통신/상태 조합/필터 계산은 feature 하위 `hooks/`로 분리한다.
3. 폼/테이블/모달 블록은 feature 하위 `components/`로 분리한다.
4. 검증 실행 설정(context/agent mode) 파싱 규칙은 `shared/utils/validationConfig.ts`를 공통으로 사용한다.
